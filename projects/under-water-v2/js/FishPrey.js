...
export default class FishPrey extends Fish {
    ...
    next(){
        ...
        const avoidSurfaceWeight = 1.5
        const avoidPredatorWeight = 2
        ...
        let avoidSurfaceForce = this.avoidSurface().mult(avoidSurfaceWeight)
        let avoidPredatorForce = this.avoidPredators().mult(avoidPredatorWeight)

        this.acceleration = new Vector(0, 0)
            .add(alignForce)
            .add(cohesionForce)
            .add(separationForce)
            .add(avoidSurfaceForce)
            .add(avoidPredatorForce)
    ...
    // Steer to avoid wave surface
    // Formula: sum of (position - wavePosition) vectors, inversely proportional to distance.
    // There are as many wave position as there is pixel in the width
    avoidSurface(){
        let sep = new Vector(0, 0)
        let total = 0
        let wavePerception = this.perception*1.5

        for(let wCoord of this.waveCoords){
            let dist = this.position.dist(wCoord)
            if(dist > 0 && dist < wavePerception){
                // vector pointing away from wave (repulsion vector), weighted by inverse distance
                let diff = this.position.copy().sub(wCoord)
                diff.div(dist)
                sep.add(diff)
                total++
            }
